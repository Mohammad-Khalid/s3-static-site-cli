...
const twilioDir = path.resolve(os.homedir(),'.aws-s3');
const filePath = path.resolve(os.homedir(),'.aws-s3','config.json');

const addProfile = async (profileName,valid_credential_object) => {
    const n_profile = {
        profileName : profileName,
        accessKeyId : valid_credential_object.accessKeyId,
        secretAccessKey : valid_credential_object.secretAccessKey,
        region : valid_credential_object.region
    };
